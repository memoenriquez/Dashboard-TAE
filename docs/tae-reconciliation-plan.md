...
- Run history table.
- Attention-required view can be the same run history filtered by failed statuses.
...
- Concurrent cron/manual generation for the same `owner_client_id + reconciled_date` relies on the unique constraint. One request creates the run; the other handles the conflict as `already_exists`. No custom distributed lock in v1.
